...
##### Manual Topology Entry

```bash
# Add a domain-to-port relationship
tox -e cli -- add-edge --domain example.com --port 443 --protocol TCP
tox -e cli -- add-edge --domain example.com --port 53 --protocol UDP

# Add a host (physical or virtual machine)
tox -e cli -- add-host --ip 192.168.1.1 --hostname web.example.com --os-type Linux

# Add a DNS resolution edge (domain resolves to host IP)
tox -e cli -- add-dns-resolves-to-host --domain example.com --ip 192.168.1.1

# Short form
tox -e cli -- add-edge -d api.example.com -p 8080
tox -e cli -- add-host -i 10.0.0.5 -h api.example.com -o Ubuntu
tox -e cli -- add-dns-resolves-to-host -d api.example.com -i 10.0.0.5
```
...
# Discover specific record types
tox -e cli -- discover-dns --domain example.com --type A,AAAA,MX

# Short form with specific types
tox -e cli -- discover-dns -d api.example.com -t A -t CNAME

# View help
tox -e cli -- discover-dns --help
```

##### Subdomain Discovery

```bash
# Discover common subdomains for a domain (uses built-in 108-subdomain list)
tox -e cli -- discover-subdomains --domain example.com

# Short form
tox -e cli -- discover-subdomains -d example.com

# Use custom subdomain dictionary file
tox -e cli -- discover-subdomains --domain example.com --dictionary-file /path/to/subdomains.txt

# Short form with dictionary file
tox -e cli -- discover-subdomains -d example.com -f subdomains.txt

# View help
tox -e cli -- discover-subdomains --help
```

###### Custom Dictionary File Format

The `--dictionary-file` option accepts a plain text file with one subdomain per line:
...
- Whitespace is automatically trimmed from each line
...
# Short form
tox -e cli -- scan-ports -t 10.0.0.1 -p 1-65535

# View help
tox -e cli -- scan-ports --help
```
...
tox -e cli -- add-edge --help
tox -e cli -- add-host --help
tox -e cli -- add-dns-resolves-to-host --help
```

#### REST API

All CLI commands are also available via REST API endpoints with HTTP Basic authentication.
...
```bash
# Create a host
curl -u admin:password -X POST http://localhost:8000/api/v1/hosts \
  -H "Content-Type: application/json" \
  -d '{"ip_address": "192.168.1.100", "hostname": "server.example.com", "os_type": "Linux"}'
...
# Discover subdomains
curl -u admin:password -X POST http://localhost:8000/api/v1/discover/subdomains \
  -H "Content-Type: application/json" \
...
```bash
# Run topology CRUD tests
./test-scripts/test_topology_api.sh

# Run discovery & scanning tests
./test-scripts/test_discovery_api.sh

# Test against custom server with custom credentials
VIA_NODE_USER=myuser VIA_NODE_PASSWORD=mypass ./test-scripts/test_topology_api.sh http://server:8080
```

#### Running Tests

```bash
# Run all tests with coverage and code quality checks
tox

# Run only the test suite (no linting)
tox -e default -k test

# Run tests in watch mode (auto-run on file changes)
tox -e watch

# Run specific test file
tox -e default -- tests/via_node/domain/model/test_host.py

# Run tests matching a pattern
tox -e default -- -k "test_host" -v
```
...
# Run all linters (flake8, mypy, bandit, xenon, semgrep)
tox -e lint
 ...
 interface/                         # APIs and CLI
    api/
        ...
        data_transfer_object/
            ...
            edge_dto.py
            dns_record_dto.py
            dns_discovery_dto.py
            subdomain_discovery_dto.py
            port_scan_dto.py
